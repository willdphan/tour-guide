...
Able to recognize elements not visible on page.
...
def generate_personable_instruction(action, element_description, text_input):
    llm = ChatOpenAI(model="gpt-3.5-turbo", temperature=0.7)
    response = llm.predict(personable_prompt.format(action=action, element_description=element_description, text_input=text_input))
    return response.strip()
