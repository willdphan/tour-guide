      ...
      if (param.type === "ObjectPattern") {
        param.properties.forEach((prop: any) => {
          if (prop.type === "ObjectProperty") {
            ...
            const propType =
              prop.value.typeAnnotation?.typeAnnotation.type || "any";
          ...
          if (propType.type === "TSTypeLiteral") {
            propType.members.forEach((member: any) => {
              if (member.type === "TSPropertySignature") {
                ...
                const propType =
                  (member.typeAnnotation as any)?.typeAnnotation?.type || "any";
                props.push({ name: propName, type: propType });
              }
            });
          }
  ...
  traverse(ast, {
    ...
    JSXOpeningElement(path) {
      ...
      const role = attributes.find(
        (attr: any) => attr.type === "JSXAttribute" && attr.name.name === "role"
      );
      const ariaAttributes = attributes.filter(
        (attr: any) =>
  ...
  routePatterns.forEach((pattern) => {
    ...
    files.forEach((filePath) => {
          ...
          if (initializer && Node.isArrayLiteralExpression(initializer)) {
            initializer.getElements().forEach((element) => {
              if (Node.isObjectLiteralExpression(element)) {
                ...
                const componentProp =
                  element.getProperty("component") ||
                  element.getProperty("element");

                if (
                  pathProp &&
                  componentProp &&
                  Node.isPropertyAssignment(pathProp) &&
                  ...
                  if (path && component) {
                    ...
                    if (guardProp && Node.isPropertyAssignment(guardProp)) {
                      route.guards = [
                        guardProp.getInitializer()?.getText() || "",
                      ];
                    }
      ...
      if (defaultExport) {
        ...
        const routePath =
          "/" +
          relativePath.replace(/\.(tsx|ts|js|jsx)$/, "").replace(/index$/, "");
        ...
        ExportDefaultDeclaration(path) {
          if (path.node.declaration.type === "ObjectExpression") {
            ...
            methods.forEach((method) => {
              const prop = path.node.declaration.properties.find(
                (p) => p.key.name === method
              );
              if (prop) {
                ...
                if (
                  prop.value.type === "Function Expression" ||
                  ...
                  if (
                    ...
                    prop.value.returnType.typeAnnotation
                  ) {
...
async function extractSeoMetadata(
  rootDir: string
        ...
        if (path.node.openingElement.name.name === "Head") {
          path.node.children.forEach((child: any) => {
            if (
              ...
              const nameProp = child.openingElement.attributes.find(
                (attr: any) => attr.name.name === "name"
              );
              const contentProp = child.openingElement.attributes.find(
                (attr: any) => attr.name.name === "content"
              );
...
function createDependencyGraph(components: ComponentMetadata[]): {
  [key: string]: string[];
} {
...
async function extractNavigationStructure(
  rootDir: string
      ...
      traverse(ast, {
        JSXElement(path) {
          if (
            ...
            path.node.openingElement.name.name === "NavLink"
          ) {
            const hrefAttr = path.node.openingElement.attributes.find(
              (attr: any) => attr.name.name === "href"
            );
            const labelNode = path.node.children.find(
              (child: any) =>
                child.type === "JSXText" || child.type === "StringLiteral"
            );

            if (hrefAttr && labelNode) {
              const href = (hrefAttr.value as any).value;
              const label =
                labelNode.type === "JSXText"
...
async function analyzeGlobalStyles(rootDir: string): Promise<string[]> {
  ...
  const styleFiles = glob.sync(
    path.join(rootDir, "src", "**", "*.{css,scss,less}")
  );
...
async function generateAppMetadata(rootDir: string): Promise<AppMetadata> {
  ...
  for (const route of routes) {
    const pageMetadata = await analyzePage(
...
async function generateMetadataFile(rootDir: string) {
  try {
    ...
    const outputDir =
      "/Users/williamphan/Desktop/tourguide/src/app/sdk/metadata";
