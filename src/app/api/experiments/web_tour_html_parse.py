...
Parses the entire website prior and saves to vector db prior to query to prevent constant parsing.

WHY IT DOESN'T WORK:
- doesn't have enoguh context of where user is
- incorporates external info in addition to parsed data
- many state changes occur, unable to recognize this unless u parse constantly
- 50% quicker than constant parsing though
"""
...
index_name = os.getenv("PINECONE_INDEX_NAME", "tour")
...
with open("parsed_website_data.pkl", "rb") as f:
...
async def parse_entire_website(start_url):
    async with async_playwright() as p:
        ...
        try:
            ...
            parsed_data[page.url] = await enhanced_content_analysis(page)
            ...
            # Parse each linked page
            for link in links:
                if link.startswith(start_url):  # Only parse pages from the same domain
                    try:
...
def upsert_to_pinecone(parsed_data):
    batch_size = 100
    for i in range(0, len(parsed_data), batch_size):
        batch = list(parsed_data.items())[i:i+batch_size]
        to_upsert = []
        for url, page_info in batch:
            embedding = create_embeddings(page_info['text_content'])
            to_upsert.append((url, embedding, {"title": page_info['title']}))
...
def retrieve_relevant_documents(question, top_k=3):
    ...
    for result in results['matches']:
        ...
        relevant_docs.append({
            ...
            "content": parsed_data[url]['text_content'][:200] + "..."  # Truncate for brevity
        })
    return relevant_docs

def extract_navigation_structure(parsed_data):
    home_page = parsed_data.get('http://localhost:3000', {}).get('content', '')
    soup = BeautifulSoup(home_page, 'html.parser')
    nav_items = soup.find_all('a', class_='nav-item')
    navigation = [item.text.strip() for item in nav_items if item.text.strip()]
    return navigation

def generate_response(question, user_state):
    relevant_docs = retrieve_relevant_documents(question)
    navigation = extract_navigation_structure(parsed_data)
    current_url = user_state.get('current_url', 'http://localhost:3000')
    current_page_title = parsed_data.get(current_url, {}).get('title', 'Unknown Page')
    
    prompt = f"""
    You are a helpful guide for a website about stools and furniture. The user is currently on the page titled "{current_page_title}" with URL {current_url}. They need directions to find information or navigate to specific pages. Based on the following information:

    User's question: {question}

    User's current location: {current_page_title} (URL: {current_url})

    Website structure:
    Home page navigation items: {', '.join(navigation)}

    Relevant pages:
    {' '.join([f"Page: {doc['title']}, Content: {doc['content']}" for doc in relevant_docs])}

    Please provide a concise and helpful response that guides the user to the most relevant information or page. Follow these guidelines:
    1. Consider the user's current location on the website when providing directions.
    2. Use the provided navigation structure to guide the user.
    3. Provide step-by-step instructions on how to navigate the website to find the information.
    4. If the information is about a specific product, mention its name and how to find it in the navigation or product listings.
    5. If no relevant information is found, suggest alternative ways to browse or search the website using the available navigation options.
    6. Keep the tone friendly and conversational. ANSWER ONLY BASED ON THE NAVIGATION ITEMS, DO NOT ANSWER BASED OFF OF OUTSIDE CONTEXT/KNOWLEDGE. No extra information. Keep it concise.
    ...
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You are a helpful assistant guiding users through a website about stools and furniture."},
...
async def run_agent(question: str, page=None, user_state=None):
    try:
        if page is None:
            ...
            async with async_playwright() as p:
                ...
                try:
                    async for step in _run_agent_with_page(question, page, user_state):
                        yield step
                finally:
                    await browser.close()
        else:
...
async def _run_agent_with_page(question: str, page, user_state=None):
    # Use the parsed data instead of navigating to the page
    current_url = user_state.get('current_url', "http://localhost:3000")
    current_page_info = parsed_data.get(current_url, {})
    
    # Use the pre-parsed content instead of fetching it live
    ...
    # Update initial_state with user_state if provided
    if user_state:
        initial_state.update(user_state)
    ...
    async for event in event_stream:
        ...
        try:
            if action == "Click":
                ...
                # Yield the final state before breaking
    ...
    # If we haven't yielded the final state yet (e.g., if we didn't hit an ANSWER action), yield it now
    if not action.startswith("ANSWER"):
        yield {"final_state": state}
...
async def main():
    # Check if data is already in Pinecone
    if index.describe_index_stats()['total_vector_count'] == 0:
        print("Upserting data to Pinecone...")
        upsert_to_pinecone(parsed_data)
...
def generate_response(question, relevant_docs, parsed_data, user_state):
    navigation = extract_navigation_structure(parsed_data)
    current_url = user_state.get('current_url', 'http://localhost:3000')
    current_page_title = parsed_data.get(current_url, {}).get('title', 'Unknown Page')
