...
function PopUpDefault({ action = {}, onConfirm, isAgentRunning, isWaiting }: AgentActionConfirmationProps) {
  ...
  useEffect(() => {
    ...
    }, 3000) // Check every 4 seconds if we need to restart the animation
  ...
  const getPhaseColor = (phase: string) => {
    switch (phase) {
      case 'Initializing': return '#4ADE80' // Green-400
      case 'Analyzing': return '#22C55E' // Green-500
      case 'Processing': return '#16A34A' // Green-600
      case 'Finalizing': return '#15803D' // Green-700
      ...
      <AnimatePresence>
        {notifications.map((notification, index) => (
          <motion.div
            ...
            className={`absolute bottom-16 left-0 right-0 shadow-md ${ibmPlexSans.className} ${
              isDarkMode ? 'bg-black bg-opacity-80 text-white' : 'bg-white text-gray-800'
            }`}
            style={{ 
            ...
            animate={{ 
              opacity: 1, 
              y: index * 15, // Offset each notification slightly
              scale: 1 - index * 0.02 // Slightly scale down each notification
            }}
              ...
              <div className="flex justify-between items-center mb-2">
                <motion.div className="flex items-center space-x-2">
                  <div
                    ...
                    style={{ backgroundColor: getPhaseColor(notification) }}
                  >
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" className="relative z-10">
                      <motion.g animate={getEyeAnimation()}>
                        <motion.rect x="4" y="5" width="2" height="6" rx="1" fill="white" animate={getBlinkAnimation()} />
                        <motion.rect x="10" y="5" width="2" height="6" rx="1" fill="white" animate={getBlinkAnimation()} />
                      </motion.g>
                    </svg>
                  </div>
              ...
              <div className="space-y-2">
                <div className={`h-1 w-full ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'} overflow-hidden`}>
                  <motion.div
                    ...
                    animate={{
                      width: `${progress}%`,
                      backgroundColor: getPhaseColor(notification)
                    }}
                ...
                <div className="flex justify-between items-center">
                  ...
                  <motion.span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    {progress < 100 ? "In progress" : "Complete"}
                  </motion.span>
                </div>
              </div>
              <motion.p className="mt-2 text-xs">
                {action?.instruction || "Processing..."}
              </motion.p>
              {action?.thought && (
                <motion.p className={`mt-1 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} leading-relaxed`}>
                  {action.thought}
                </motion.p>
              )}
            </div>
...
export default function Component() {
  ...
  <Link href="#" className="text-xs text-gray-500 flex items-center">
    Read Story <ArrowUpRight className="w-4 h-4 ml-1" />
                ...
                <div className="flex items-center space-x-4 border-b-[1px] border-black pb-4 pt-4 font-Marcellus text-xs">
                  <div className="w-12 h-12 relative">
                    <div className="w-full h-full rounded-full bg-[#15803D] flex items-center justify-center overflow-hidden">
                ...
                <div className="flex space-x-4 pt-12">
                  ...
                  <Link href="#" className="border px-6 py-3 border-black rounded-full text-sm font-medium">
                    Our Process
                  </Link>
                </div>
              </div>
              ...
              <div className="lg:w-1/2 mt-12 lg:mt-0">
                <div className="relative aspect-square max-w-lg mx-auto">
                  ...
                  <div className="absolute inset-x-0 bottom-0 left-40 flex justify-center">
                    <PopUpDefault
                      action={{}}
                      onConfirm={() => {}}
        ...
        <div className="py-14">
            <div className="max-w-screen-xl mx-auto px-4 md:px-8">
                {/* <h3 className="font-Marcellus text-sm text-black font-bold text-center">
                  Here for some fake clout.
                </h3> */}
                <div className="mt-6">
