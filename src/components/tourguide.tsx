...
const AgentGuide: React.FC = () => {
  ...
  const [manualCoordinates, setManualCoordinates] = useState({ x: 0, y: 0 });
  ...
  const handleManualCoordinateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setManualCoordinates(prev => ({ ...prev, [name]: parseFloat(value) }));
  };

  const handleManualPlot = (e: React.FormEvent) => {
    e.preventDefault();
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-6 flex flex-col justify-center sm:py-12">
      ...
      {/* Add this new form for manual coordinate input */}
      <div className="mt-8 border-t pt-8">
        <h2 className="text-xl font-semibold mb-4">Manual Coordinate Plotting</h2>
        <form onSubmit={handleManualPlot} className="space-y-4">
          <div>
      ...
      {/* Highlight for manual coordinates */}
      {showManualHighlight && (
        <motion.div
          className="fixed border-2 border-blue-500 pointer-events-none z-50"
          style={{
            left: `${manualCoordinates.x}px`,
            top: `${manualCoordinates.y}px`,
            width: '20px',
            height: '20px',
          }}
          initial={{ opacity: 0 }}
